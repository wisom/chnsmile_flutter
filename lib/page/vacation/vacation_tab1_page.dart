...
import 'package:chnsmile_flutter/widget/vacation_card1.dart';
import 'package:flutter/material.dart';
import 'package:flutter_boost/flutter_boost.dart';
import 'package:flutter_easyloading/flutter_easyloading.dart';

class VacationTab1Page extends StatefulWidget {
  const VacationTab1Page({Key key}) : super(key: key);

  @override
  _VacationTab1PageState createState() => _VacationTab1PageState();
}

class _VacationTab1PageState
    extends OABaseTabState<VacationModel1, Vacation, VacationTab1Page> {
  var isLoaded = false;
  ...
  get contentChild => dataList.isNotEmpty
      ? ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) =>
              VacationCard1(item: dataList[index], onCellClick: _onCellClick))
  ...
  void _onCellClick(Vacation item) {
    if (getEditStatus(item.status)) {
      ...
      BoostNavigator.instance
          .push('vacation_detail_page', arguments: {"id": item.id, "type": 1});
    }
  }
  ...
  Future<VacationModel1> getData(int pageIndex) async {
    ...
    try {
      ...
      VacationModel1 result =
          await VacationDao.get(type: 1, pageIndex: pageIndex, pageSize: 10);
