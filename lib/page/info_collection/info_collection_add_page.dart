...
class InfoCollectionAddPage extends StatefulWidget {
  final Map params;
  String id;

  InfoCollectionAddPage({Key key, this.params}) : super(key: key) {
    id = params['id'];
  }

  @override
  _InfoCollectionAddPageState createState() => _InfoCollectionAddPageState();
}

class _InfoCollectionAddPageState extends HiState<InfoCollectionAddPage> {
  ...
  _buildAppBar() {
    ...
    return appBar(isEdit ? '文件发起' : '文件新建', rightTitle: rightTitle, rightButtonClick: () {
      if (rightTitle == '删除') {
        delete();
      }
    });
