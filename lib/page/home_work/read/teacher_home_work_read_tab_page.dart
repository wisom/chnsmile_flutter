...
class TeacherHomeWorkReadTabPage extends StatefulWidget {
  String id;
  String classId;
  String typeText;
  int type;
  ...
  TeacherHomeWorkReadTabPage(
      {Key key, this.id, this.classId, this.type, this.typeText, this.status})
      : super(key: key);

  @override
  _TeacherHomeWorkReadTabPageState createState() =>
      _TeacherHomeWorkReadTabPageState();
}

class _TeacherHomeWorkReadTabPageState
    extends OABaseTabState<NoticeReadModel, Read, TeacherHomeWorkReadTabPage> {
  var isLoaded = false;
  ...
  remind() async {
    try {
      ...
      var result = await HomeWorkDao.teacherRemind(params);
      showWarnToast('已提醒');
  ...
  Future<NoticeReadModel> getData(int pageIndex) async {
    ...
    NoticeReadModel result =
        await HomeWorkDao.readList(widget.id, widget.classId);
    EasyLoading.dismiss(animation: false);
    isLoaded = false;
    return result;
  }
